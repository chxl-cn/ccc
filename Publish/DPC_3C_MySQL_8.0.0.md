...
主要性能目标，设计容量500台车，每天10万条报警，72万条状态日志；Alarm和C3_SMS按天建分区;100人同时在线：
...
2. 报警检索场景和指标
    ...
    5. 统计日报：考虑用中间表预汇总或Redis缓存数据
    6. 统计月报：考虑用中间表预汇总或Redis缓存数据
    7. 统计周报：考虑用中间表预汇总或Redis缓存数据
    8. 重复报警分析：考虑用中间表预汇总或Redis缓存数据？
3. 报警详情处理指标
