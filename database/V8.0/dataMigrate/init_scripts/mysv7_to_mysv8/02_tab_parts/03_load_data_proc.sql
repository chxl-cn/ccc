...
BEGIN
    ...
    BEGIN
        ...
        THEN
            ...
            DELETE FROM nos_aa_pnew p WHERE p.INPUTDATE >= p_date AND p.INPUTDATE < current_date + INTERVAL 1 DAY;
            ...
            SELECT r.*
                FROM alarm                 l
                         INNER JOIN nos_aa r
                                    ON l.id = r.ID AND l.raised_time >= p_date AND l.raised_time < current_date + INTERVAL 1 DAY;
    ...
    SELECT concat(f, fv, t, tv, d) FROM wv_load WHERE tv = 'c3_sms_monitor_pnew' INTO OUTFILE 'd:/loaddir/c3_sms_monitor/load_c3_sms_monitor.sql';
END //