DELIMITER ;
DROP PROCEDURE IF EXISTS loco_stats;

DELIMITER //
CREATE PROCEDURE loco_stats(IN p_bureau_code      VARCHAR(100)
                           , IN p_org_code        VARCHAR(100)
                           , IN p_p_org_code      VARCHAR(100)
                           , IN p_locomotive_code VARCHAR(100)
                           , IN p_stime           DATETIME
                           , IN p_etime           DATETIME
...
BEGIN
    ...
    SET v_where = concat(v_where, char(10), if(p_position_code IS NOT NULL, CONCAT("and position_code like '", p_position_code, "'"), v_space));
    ...
    SET v_where = concat(v_where, char(10), if(p_routing_no IS NOT NULL, CONCAT("and routing_no = ", p_routing_no), v_space));
